...
//? 특징
//@ - 컴포넌트 단위에서의 상태 관리
//@ - '상태값(state)과' '해당 상태를 업데이트하는 함수(setState)'를 한 쌍으로 변환
...
// const [state, setState] = useState(); - 함수 컴포넌트 밖에서 사용할수 없음
...
function State01() {
  ...
  // 명명 규칙 : set + 상태명 (name -> setName, const -> setCount, post -> setPost)
  ...
  const handleUpClick = () => {
    ...
    // cf) set- 상태 변경 함수 내에서 콜백 함수(콜백 함수= 인자값)
    //     setCount(() => {})
    //    >> 해당 콜백 함수의 인자는 '상태의 최신값!'
    //    >> prev-상태명 (prevName, prevCount, prevUser / previous 이전의)
    setCount(prevCount => prevCount + 1); // 0 + 1 === 1(최신값)
    setCount(prevCount => prevCount + 1); // 1(최신값) + 1(현재값) === 2
  }
